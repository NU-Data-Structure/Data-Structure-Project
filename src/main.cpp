...
int main() {
    ...
    // Initialize globalOrderID from CSV
    try {
        ...
        for(int id : ids) {
            if(id > globalOrderID) globalOrderID = id;
        }
        cout << "Initialized globalOrderID to " << globalOrderID << endl;
    } catch(...) {}
...
json response = {
    ...
    svr.Post("/api/cart/checkout", [&](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            if (j.contains("roomNumber")) {
                newOrder.roomNumber = j["roomNumber"];
            } else {
                newOrder.roomNumber = "";
            }
    ...
    svr.Post("/api/admin/processOrder", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            vector<int> custIds = doc.GetColumn<int>("CustomerID");
            vector<string> providers = doc.GetColumn<string>("Provider");
            int rowIndex = -1;
            for(size_t i=0; i<ids.size(); ++i) {
                bool idMatch = (ids[i] == processedOrder.orderId);
                bool statusMatch = (statuses[i] == "Pending" || statuses[i] == "pending");
                bool custMatch = (custIds[i] == processedOrder.customerId);
                bool providerMatch = (providers[i] == processedOrder.provider);

                if(idMatch && statusMatch && custMatch && providerMatch) {
                    rowIndex = i;
                    break;
                }
            }
