...
async function loadQueue() {
    ...
    try {
        ...
        if (data.success) {
            ...
            let html = '<table><thead><tr><th>Order ID</th><th>Customer ID</th><th>Products</th><th>Total</th><th>Status</th><th>Room</th></tr></thead><tbody>';
    ...
    try {
        ...
        if (confirm(summary + "Proceed with checkout?")) {
            ...
            if (visaRadio && visaRadio.checked) {
                ...
                // Store minimal data needed for payment page
                localStorage.setItem('pendingCustomerId', customerId);
                localStorage.setItem('pendingPaymentMethod', 'Visa');
                const roomNumber = document.getElementById("roomNumber").value;
                localStorage.setItem('pendingRoomNumber', roomNumber);
...
// Clean up any pending payment data when not on payment page
// Clean up any pending payment data when not on payment page
function cleanupPaymentData() {
    if (!window.location.pathname.includes('payment.html')) {
        ...
        localStorage.removeItem('pendingRoomNumber');
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Update cart display on relevant pages
...
// Clean up pending cart data if user navigates away
window.addEventListener('beforeunload', function () {
    // Only clear if not on payment page
    if (!window.location.pathname.includes('payment.html')) {
        localStorage.removeItem('pendingCartData');
    }
});

// Check for pending cart data on page load
document.addEventListener('DOMContentLoaded', function () {
    // If on cart page with pending Visa payment, warn user
    if (window.location.pathname.includes('cart.html')) {
        const pendingCartData = localStorage.getItem('pendingCartData');
        if (pendingCartData) {
            if (confirm('You have a pending Visa payment. Would you like to continue to payment?')) {
                window.location.href = 'payment.html';
            } else {
                localStorage.removeItem('pendingCartData');
            }
    ...
    // If on profile page with success parameter, show success message
    if (window.location.pathname.includes('profile.html')) {
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('payment') === 'success') {
            const orderId = urlParams.get('orderId');
            if (orderId) {
                alert(`✅ Order #${orderId} placed successfully with Visa payment!`);
                // Clean URL
